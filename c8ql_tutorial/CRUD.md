...
<!-- ref to C8QL introduction about DML -->
...
Note: C8QL does not permit multiple `INSERT` operations that target the same collection in in a single query. <!-- explain why? --> It is allowed as body of a `FOR` loop however, inserting multiple documents like we did with above query.
