...
Despite few differences,

> Anyone with an SQL background should have no difficulty in learning C8QL.
...
* **Note:** In C8QL the preferred way is to `return` the document parts from the different collections in individual sub-attributes to avoid attribute name conflicts, e.g.:
...
C8QL is thus a little bit more complex than plain SQL at first, but offers much
...
The C8QL `RETURN` statement returns one item per document it is handed. You can
return the whole document, or just parts of it. 

Given that *oneDocument* is a document (retrieved like `LET oneDocument = DOCUMENT("myusers/3456789")` for instance), it can be returned as-is like this:
...
More complex [array](../C8QL/Functions/Array.md) and [object manipulations](../C8QL/Functions/Document.md) can be done using C8QL functions and [operators](../C8QL/Operations/README.md).
